Const WaysFolderPath = "..\rail_tracks\"
Const BridgesFolderPath = "..\rail_bridges\"
Const ElevatedFolderPath = "..\rail_elevated\"
Const TunnelsFolderPath = "..\rail_tunnels\"
Const Author = "Fabio Gonella"

Dim FSO, Folder, File, WayType, Path, V
Dim ScriptLog, Args, Execute, Opti
		...
		Case "png"
			...
			If FSO.FileExists(Path) Then
				...
				Else
					ScriptLog(0) = ScriptLog(0) & File.Name & vbNewLine
					If Execute Then
						If Opti Then OptiPng File.Path
						FSO.DeleteFile(Path)
						File.Move Path
					End If
				End If
